    ...
    <view class="list">
      <view class="card" v-for="item in list" :key="item.id" @click="goQuestion(item)">
        <view class="card-top">
          ...
          <text class="related-time" v-if="item.relatedTime">{{ item.relatedTime }}</text>
        </view>
        <text class="content line-2">{{ item.content }}</text>
        <view class="chapter-name" v-if="item.chapterName">{{ item.chapterName }}</view>
      </view>
      <u-empty v-if="!loading && !list.length" text="暂无收藏" mode="data" margin-top="40"></u-empty>
...
import { getFavorites } from '../../api/practice'
...
async function loadData() {
  loading.value = true
  try {
    const data = await getFavorites(courseId.value)
    list.value = data
...
.related-time { font-size: 11px; color: $text-3; }
